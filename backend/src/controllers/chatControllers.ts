...
] as ChatCompletionStreamRequestMessages[]
...
export async function conversation(req: Request, res: Response, next: NextFunction) {
    ...
    try {
        ...
        const chatHistory = user.chats.map(({ role, content }) => ({ role, content })) as ChatCompletionStreamRequestMessages[]
...
export async function getAllChats(req: Request, res: Response, next: NextFunction) {
    try {
        const user = await User.findById(res.locals.jwData.id)
        if (!user) {
            return res.status(401).json({ message: "Please login to continue" })
